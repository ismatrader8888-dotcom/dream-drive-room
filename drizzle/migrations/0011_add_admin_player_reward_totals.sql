...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'profiles', (SELECT coalesce(jsonb_agg(jsonb_build_object(
      ...
      'vehicleRewardsToday', coalesce((SELECT sum(amount) FROM public.vehicle_reward_events e WHERE e.user_id = p.id AND (e.earned_at AT TIME ZONE 'America/Sao_Paulo')::date = (now() AT TIME ZONE 'America/Sao_Paulo')::date), 0),
