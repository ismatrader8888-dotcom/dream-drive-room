...
BEGIN
  ...
  SELECT count(*)::integer INTO total_invited_count
  ...
  WHERE r.referrer_id = current_user_id
    AND EXISTS (
      ...
      WHERE pc.user_id = r.referred_user_id
        AND pc.status = 'CONFIRMED'
    )
    AND EXISTS (
      SELECT 1
      FROM public.user_vehicles uv
      WHERE uv.user_id = r.referred_user_id
    );
  ...
  RETURN jsonb_build_object(
    'completed', LEAST(qualified_count, 3),
    'totalInvited', total_invited_count,
    'qualifiedInvited', qualified_count,
    'goal', 3,
    'reward', 20,
