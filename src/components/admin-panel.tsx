...
type Profile = { id: string; email: string | null; phone: string | null; balance: number; rewardBalance: number; inviteCode: string; referrals: number };
type RequestRow = { id: string; userId: string; email: string | null; amount: number; status: string; createdAt: string; pixKey?: string };
type PixCharge = RequestRow & { payerName: string; magicId: string | null; creditedAt: string | null };
...
type Dashboard = { users: number; gameCredits: number; rewardBalance: number; purchases: number; pendingWithdrawals: number; pendingPix: number; profiles: Profile[]; withdrawals: RequestRow[]; pixCharges: PixCharge[]; purchasesList: Purchase[]; popularVehicles: Popular[] };
    ...
    <div className="min-h-screen bg-background pb-10">
      ...
      {!data ? <p className="p-8 text-center text-muted-foreground">Carregando dados...</p> : <>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {tab === "users" && data.profiles.map((profile) => <article key={profile.id} className="rounded-lg bg-card p-5 shadow-card"><b className="block truncate">{profile.email ?? profile.phone ?? "Sem identificação"}</b><div className="mt-3 grid grid-cols-3 gap-2 text-sm text-muted-foreground"><span>Créditos: <strong className="block text-foreground">{money(profile.balance)}</strong></span><span>Prêmios: <strong className="block text-foreground">{money(profile.rewardBalance)}</strong></span><span>Indicados: <strong className="block text-foreground">{profile.referrals}</strong></span></div><p className="mt-3 text-xs text-muted-foreground">Código: {profile.inviteCode}</p><Button className="mt-4 w-full" disabled={busy === profile.id} onClick={() => void adjust(profile)}>Alterar carteira</Button></article>)}
