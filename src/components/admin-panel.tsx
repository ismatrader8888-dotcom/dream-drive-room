...
type PixCharge = RequestRow & { payerName: string; magicId: string | null; creditedAt: string | null; refereeBonus: number; referrerBonus: number; secondLevelBonus: number };
    ...
    <div className="min-h-screen bg-background pb-10">
      ...
      {!data ? <p className="p-8 text-center text-muted-foreground">Carregando dados...</p> : <>
        ...
        <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <Metric icon={WalletCards} label="Creditado em prêmios" value={money(data.vehicleRewardsTransferred)} />
        </section>
...
function PixCharges({ rows }: { rows: PixCharge[] }) {
  ...
  return <>{rows.map((row) => <article key={row.id} className="rounded-lg bg-card p-4 shadow-card"><div className="flex justify-between gap-4"><div className="min-w-0"><b className="block truncate">{row.payerName}</b><p className="truncate text-sm text-muted-foreground">{row.email ?? "Usuário"}</p><p className="mt-1 text-xs text-muted-foreground">{row.magicId ?? "Gerando cobrança"}</p>{(row.refereeBonus > 0 || row.referrerBonus > 0 || row.secondLevelBonus > 0) && <p className="mt-2 text-xs text-primary">Bônus: convidado {money(row.refereeBonus)} · nível 1 {money(row.referrerBonus)} · nível 2 {money(row.secondLevelBonus)}</p>}</div><div className="text-right"><b>{money(row.amount)}</b><p className={`mt-1 text-xs font-semibold ${row.status === "CONFIRMED" ? "text-primary" : "text-muted-foreground"}`}>{row.status}</p></div></div></article>)}</>;
}
